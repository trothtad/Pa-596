...
**Base terrain is a static image:**
- Single Sprite2D loaded once, never redrawn
- Satellite/aerial imagery with stylization filter
- TerrainData grid is invisible — logic only, not visual

**Custom `_draw()` is reserved for:**
- Fog of war overlay (shader or draw layer)
- Dynamic damage overlays (craters, rubble, fortifications)
- Path preview lines
- Debug visualizations (detection radii, LOS lines)

**Not for:** Base terrain (static image), entity sprites, health bars, selection highlights.
...
│   ├── sprites/                # Entity sprites (soldiers, hounds)
│   ├── maps/                   # Static map images (satellite/aerial)
│   ├── overlays/               # Dynamic damage sprites (crater, rubble)
│   └── audio/
│
├── data/                       # JSON databases (NEW)
│   ├── base/                   # Core game data
│   │   ├── weapons.json
│   │   ├── weapons.schema.json # Validation schema
│   │   ├── soldiers.json
│   │   ├── soldiers.schema.json
│   │   ├── squads.json
│   │   ├── squads.schema.json
│   │   ├── hostiles.json
│   │   └── hostiles.schema.json
│   ├── terrain/                # Terrain type definitions
│   │   ├── terrain_types.json
│   │   └── terrain_types.schema.json
│   └── effects/                # Status effects, environmental
│       ├── effects.json
│       └── effects.schema.json
│
├── maps/                       # Battle maps (NEW)
│   └── kapiti_coast/           # Each map is a folder
│       ├── image.png           # Static background (satellite/aerial)
│       ├── terrain.json        # TerrainData grid (invisible logic)
│       └── meta.json           # Map name, size, spawn zones, etc.
│
...
### 3.1 Weapons Database

**Schema** (`data/base/weapons.schema.json`):
```json
{
  "required": ["display_name", "type", "damage", "effective_range", "magazine_size"],
  "known_keys": [
    "display_name", "type", "caliber", "magazine_size", "rate_of_fire",
    "base_accuracy", "effective_range", "maximum_range", "damage",
    "noise_radius", "weight_kg", "suppression_factor", "requires_setup"
  ]
}
```

**Data** (`data/base/weapons.json`):
```json
{
  "lee_enfield": {
...
Instead of assembling a fresh context dictionary every tick, maintain persistent **blackboards** that systems write to and AI reads from. Two tiers:
...
```gdscript
# Owned by BattleManager
var entity_blackboards: Dictionary = {}   # {entity_id: EntityBlackboard}
var faction_blackboards: Dictionary = {}  # {faction_id: FactionBlackboard}
...
class EntityBlackboard:
    ...
    var detected_target_id: int = -1  # Entity ID, -1 if none
    ...
    # Self-assessment (entity writes own state):
    var health_ratio: float = 1.0
    var ammo_ratio: float = 1.0
    
    # Spatial queries write:
...
class FactionBlackboard:
    ...
    # Shared detection intel:
    var known_enemy_positions: Dictionary = {}  # {entity_id: Vector2}
    var alert_level: int = 0  # 0=unaware, 1=suspicious, 2=alert, 3=combat
    
    # Tactical state:
    var suppression_zones: Array[Vector2] = []  # Areas under heavy fire
    var rally_point: Vector2 = Vector2.ZERO
    
    # Pack behavior (for hounds):
    var pack_leader_id: int = -1
    var coordinated_target_id: int = -1
```

**Write Timing — Tick Phase Order:**

All systems follow this order to prevent mid-tick reads of half-updated state:

```
Tick N:
  1. SENSE PHASE    — Detection, spatial queries write to blackboards
  2. THINK PHASE    — AI reads blackboards, decides actions, queues commands
  3. ACT PHASE      — Movement executes, attacks resolve
  4. CLEANUP PHASE  — Dead entities removed, stale blackboard data scrubbed
```

**Stale Data Handling:**

When entity 50 dies but another entity's blackboard says `detected_target_id: 50`:

1. **Lookup validates:** `BattleManager.get_entity(id)` returns `null` if dead. Callers handle gracefully:
   ```gdscript
   var target := BattleManager.get_entity(blackboard.detected_target_id)
   if target == null:
       blackboard.detected_target_id = -1  # Clear stale reference
       return  # No valid target
   ```

2. **Cleanup scrubs:** When entity dies, BattleManager iterates all blackboards and clears references to that ID.

Belt and suspenders. The lookup catches it immediately; cleanup prevents accumulation.
...
- Faction tier avoids redundant work (one detection check shares intel to all pack members)
- Deterministic phase order prevents read-during-write bugs
...
# Entity registry — BattleManager OWNS entity nodes
...
# Scene tree container — entities live here
@onready var entity_container: Node2D = $Entities  # Child node in scene

# Centralized state — two-tier blackboards
var entity_blackboards: Dictionary = {}   # {entity_id: EntityBlackboard}
var faction_blackboards: Dictionary = {}  # {faction_id: FactionBlackboard}
...
func _on_tick(tick_number: int) -> void:
    # SENSE PHASE — Systems write to blackboards
    _update_detection()      # Detection values → blackboards
    _update_spatial_queries() # Cover, distances → blackboards
    
    # THINK PHASE — AI reads blackboards, queues actions
    _update_ai()             # Reads blackboards, queues movement/attacks
    
    # ACT PHASE — Execute queued actions
    _update_movement()       # Process movement commands
    _process_combat_queue()  # Resolve attacks
    
    # CLEANUP PHASE — Remove dead, scrub stale data
    _cleanup_dead_entities() # Free nodes, clear from registry
    _scrub_stale_references() # Clear dead entity IDs from blackboards

func spawn_squad(template_id: String, position: Vector2) -> int:
    var template := Database.get_squad_template(template_id)
    if template.is_empty():
        push_error("Unknown squad template: " + template_id)
        return -1
    
    var entity_id := _next_entity_id
    _next_entity_id += 1
    
    # Create node, add to scene tree (BattleManager owns this)
    var squad_node := Squad.new()
    squad_node.entity_id = entity_id
    squad_node.squad_id = template_id
    squad_node.position = position
    entity_container.add_child(squad_node)
    
    # Register in tracking structures
    entities[entity_id] = squad_node
    squads.append(entity_id)
    entity_blackboards[entity_id] = EntityBlackboard.new()
    
    entity_spawned.emit(entity_id)
    battle_event.emit({"tick": TickManager.current_tick, "type": "entity_spawned", 
...
func despawn_entity(entity_id: int) -> void:
    ...
    var node: Node2D = entities[entity_id]
    
    # Remove from tracking
    entities.erase(entity_id)
    squads.erase(entity_id)
    hostiles.erase(entity_id)
    entity_blackboards.erase(entity_id)
    detection_map.erase(entity_id)
    
    # Free the node (BattleManager owns it, BattleManager frees it)
    node.queue_free()

func get_entity(entity_id: int) -> Node2D:
    # Returns null if entity doesn't exist (dead, never existed)
    # Callers must handle null gracefully
    return entities.get(entity_id, null)

func request_combat(attacker_id: int, target_id: int, weapon_id: String) -> void:
    # Queue combat for resolution in ACT phase
    pass
```

### 6.2 Render Manager (`scripts/managers/render_manager.gd`)

Overlay rendering only. Base terrain is a static image. Entity visuals live on entity nodes.

**Scene Structure:**
```
BattleScene
├── MapImage (Sprite2D)           # Static, loaded once, never redrawn
├── DamageOverlay (Node2D)        # Craters, rubble, fortifications
├── Entities (Node2D)             # Squads, hounds (owned by BattleManager)
├── FogOfWar (CanvasLayer/Shader) # Hides unexplored/unseen areas
└── UIOverlay (CanvasLayer)       # Path preview, selection, debug
```

**Responsibilities:**
- Load static map image on battle start
- Manage dynamic damage overlay sprites (spawn crater at position)
- Fog of war (shader recommended — samples visibility texture)
- Path preview lines (`_draw()` on UIOverlay)
- Debug visualizations

**NOT Responsibilities (handled elsewhere):**
- Base terrain (static Sprite2D, set once)
- Entity sprites (on entity nodes)
- TerrainData (invisible logic layer, no rendering)

```gdscript
extends Node2D

@onready var map_image: Sprite2D = $MapImage
@onready var damage_overlay: Node2D = $DamageOverlay

var path_preview: Array[Vector2] = []
var debug_los_lines: Array = []

func load_map(image_path: String) -> void:
    map_image.texture = load(image_path)

func spawn_crater(position: Vector2) -> void:
    var crater := Sprite2D.new()
    crater.texture = preload("res://assets/overlays/crater.png")
    crater.position = position
    damage_overlay.add_child(crater)

func spawn_rubble(position: Vector2) -> void:
    var rubble := Sprite2D.new()
    rubble.texture = preload("res://assets/overlays/rubble.png")
    rubble.position = position
    damage_overlay.add_child(rubble)
...
**Key Decisions Made:**
- JSON + schema validation for data-driven design
- Tick-based simulation (10 ticks/sec, speedable)
- Entities are Node2D children owned by BattleManager
- Two-tier blackboards (Entity + Faction) with defined write timing
- Entity IDs everywhere, no direct Node references in state
- Stale data handled by lookup validation + cleanup scrubbing
- **Static map images** — terrain visuals are a single Sprite2D (satellite/aerial imagery), TerrainData is invisible logic grid painted over the image
- **1m = 1 cell** scale — no conversion math, ranges in meters = ranges in cells
- Dynamic damage (craters, rubble) rendered as overlay sprites on top of static image

**Open Questions for Implementation:**
- Exact tick budget (how much work per tick before frame drops?)
- Soldier sprites vs placeholder shapes (when to invest in art?)
- Save/load format for mid-battle state
- Campaign layer interface (how does strategic layer spawn battles?)
- Map painting workflow (editor shows image + semi-transparent terrain overlay for classification)
- Auto-classification assist? (color → terrain type suggestions, flood fill by similarity)

**Contact:** Architecture questions → ping Red.
